...
function postDeck () {
  ...
  for (i = 0; i < deck.sideboard.length; i++) {
    cardVar = "('" + deck.sideboard[i].id + "')"
    deckDiv.innerHTML += '<div class="main-card"><p>' + deck.sideboard[i].count + 'x ' + deck.sideboard[i].card + '</p><a class="main-icon-card" uk-icon="icon: plus; ratio: 1.2" onclick="addCard' + cardVar + '"></a><a class="main-icon-card" uk-icon="icon: minus; ratio: 1.2" onclick="removeCard' + cardVar + '"></a><a class="main-icon-card" uk-icon="icon: arrow-up; ratio: 1.2" onclick="moveToSideboard' + cardVar + '"></a><br><img src="' + deck.sideboard[i].url + '"></div>'
  }
}
